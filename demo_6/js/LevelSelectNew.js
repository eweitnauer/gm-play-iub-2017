...
DinoEggs.LevelSelect.prototype = {
	...
	preload: function() {
		this.game.load.spritesheet('levelselecticons', 'assets/level_icons.png', 75, 80);
	...
	initProgressData: function() {
		...
		if (!PLAYER_DATA) {
			...
			var str = window.localStorage.getItem('game_progress');
	...
	createLevelIcon: function(xpos, ypos, levelnr, isLocked, stars) {
		...
		// add stars, if needed
		if (isLocked == false) {
            var txt = this.game.add.text(24, 16,''+levelnr, {font:"45px Arial"});
